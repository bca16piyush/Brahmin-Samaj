         ...
         <div className="container mx-auto px-4">
           ...
           {featuredEvents.length === 0 && regularEvents.length === 0 ? (
             ...
             <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
               {(featuredEvents.length > 0 ? featuredEvents : regularEvents.slice(0, 2)).map((event, index) => (
                 <motion.div
                   key={event.id}
                 ...
                 >
                   ...
                   <div className="relative z-10 p-6 lg:p-8">
                     <div className="flex flex-wrap items-center gap-2 mb-4">
                       ...
                       {event.is_featured && (
                         <Badge className="bg-gold/30 text-gold border-0">Featured</Badge>
                       )}
                     </div>
                     ...
                     {event.description && (
                       <p className="text-primary-foreground/80 mb-5 line-clamp-2 text-sm lg:text-base">
                         {event.description}
                       </p>
                     )}
           ...
           {/* More Events List */}
           {regularEvents.length > (featuredEvents.length > 0 ? 0 : 2) && (
             <div className="mt-10">
               <h3 className="font-heading text-xl font-semibold mb-6">More Upcoming Events</h3>
               <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                 {(featuredEvents.length > 0 ? regularEvents : regularEvents.slice(2)).map((event, index) => (
                   <motion.div
                     key={event.id}
                     initial={{ opacity: 0, y: 20 }}
                     whileInView={{ opacity: 1, y: 0 }}
                   ...
                   >
                     <Link
                       to={`/yagyas/${event.id}`}
                       className="block p-5 rounded-xl bg-card border border-border hover:border-primary/30 hover:shadow-lg transition-all group"
                     >
               ...
               {news.map((item, index) => (
                 ...
                 >
                   <div className="flex items-start justify-between mb-4">
                     <Badge
                       variant={item.is_urgent ? 'destructive' : 'secondary'}
                       className="gap-1"
                     >
                       {item.is_urgent && <AlertTriangle className="w-3 h-3" />}
                       {item.is_urgent ? 'Urgent' : 'Update'}
                     </Badge>
