  ...
  return <Layout>
      <section className="py-12 lg:py-20">
        <div className="container mx-auto px-4">
          ...
          {/* Happening Now */}
          {happeningNowEvents.length > 0 && <div className="mb-16">
            <h2 className="font-heading text-2xl font-bold mb-6 flex items-center gap-2">
                <span className="w-3 h-3 rounded-full bg-red-500 animate-pulse"></span>
                अभी चल रहा है
              </h2>
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                {happeningNowEvents.map((event, index) => <motion.div key={event.id} initial={{
              opacity: 0,
              y: 30
            }} animate={{
              opacity: 1,
              ...
              delay: index * 0.1
                    ...
                    {event.image_url && <div className="absolute inset-0">
                        <img src={event.image_url} alt={event.title} className="w-full h-full object-cover opacity-30" />
                    ...
                    <div className="relative z-10 p-8">
                      ...
                      {event.description && <p className="text-primary-foreground/80 mb-6 line-clamp-2">{event.description}</p>}
                      ...
                      <Link to={`/yagyas/${event.id}`}>
                        ...
                        {event.youtube_live_url && <Link to={`/yagyas/${event.id}`}>
          ...
          {featuredUpcoming.length > 0 && <div className="mb-16">
          ...
          {/* All Upcoming Events */}
          {regularUpcoming.length > 0 && <div className="mb-16">
              <h2 className="font-heading text-2xl font-bold mb-6">आगामी यज्ञ</h2>
              <div className="space-y-4">
                {regularUpcoming.map((event, index) => <motion.div key={event.id} initial={{
              opacity: 0,
              x: -20
            }} animate={{
              opacity: 1,
              ...
              delay: index * 0.1
                    ...
                    <Link to={`/yagyas/${event.id}`} className="flex flex-col md:flex-row md:items-center">
                      {/* Event Image */}
                      {event.image_url && <div className="w-full md:w-48 h-32 md:h-full shrink-0">
                      ...
                      <div className="p-6 flex flex-col md:flex-row md:items-center gap-4 flex-1">
                        ...
                        <div className="flex-1">
                          <div className="flex items-start justify-between mb-2">
                            <div>
                              <div className="flex items-center gap-2 mb-2">
                                ...
                                {event.is_live && <Badge variant="destructive" className="animate-pulse">
                                    <Video className="w-3 h-3 mr-1" />
                                    LIVE
                                  </Badge>}
                                {event.youtube_live_url && !event.is_live && <Badge variant="secondary" className="bg-red-500/20 text-red-600">
                                    <Video className="w-3 h-3 mr-1" />
                                    Video
                                  </Badge>}
                              </div>
                          ...
                          {event.description && <p className="text-sm text-muted-foreground mb-3 line-clamp-1">{event.description}</p>}
                          <div className="flex flex-wrap gap-4 text-sm text-muted-foreground">
                            ...
                            {event.registration_limit && <span className="flex items-center gap-1">
              ...
              delay: Math.min(index * 0.02, 0.5)
                    ...
                    <Link to={`/yagyas/${event.id}`}>
                      <div className="group rounded-xl bg-card border border-border hover:border-muted-foreground/30 transition-all overflow-hidden opacity-70 hover:opacity-100">
                        {event.image_url && <div className="h-32 overflow-hidden">
                            <img src={event.image_url} alt={event.title} className="w-full h-full object-cover grayscale group-hover:grayscale-0 transition-all" />
                          </div>}
                        <div className="p-4">
                          <div className="flex items-center gap-2 mb-2">
                            ...
                            {event.youtube_live_url && <Badge variant="secondary" className="text-xs bg-red-500/20 text-red-600">
                                <Video className="w-3 h-3 mr-1" />
                                Recording
                              </Badge>}
                          </div>
          ...
          {/* Empty State */}
          {!events?.length && <div className="text-center py-12">
