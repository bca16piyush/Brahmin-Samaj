  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      const {
        data,
        error
      } = await supabase.from('pandits_public').select('*').eq('is_active', true).order('name', {
        ascending: true
      });
  ...
  return useQuery({
    ...
    queryFn: async () => {
      const {
        data,
        error
      } = await supabase.from('pandit_reviews').select('pandit_id, rating');
      if (error) throw error;
      ...
      Object.entries(ratings).forEach(([panditId, {
        total,
        count
      }]) => {
        averages[panditId] = {
  ...
  return useQuery({
    ...
    queryFn: async () => {
      const {
        data,
        error
      } = await supabase.from('pandit_expertise_options').select('name').order('name', {
        ascending: true
      });
      if (error) throw error;
...
function formatAvailabilitySchedule(weeklyAvailability: WeeklyAvailability | null): {
  day: string;
  abbrev: string;
  fullName: string;
  start: string | null;
...
function StarRating({
  ...
  return <div className="flex gap-0.5">
      {[1, 2, 3, 4, 5].map(star => <Star key={star} className={`w-3.5 h-3.5 ${star <= rating ? 'fill-gold text-gold' : 'text-muted-foreground/30'}`} />)}
    </div>;
}
export default function Panditji() {
          ...
          <div className="container mx-auto px-4">
            ...
            delay: 0.3
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {[1, 2, 3, 4, 5, 6].map((_, index) => <div key={index} className="p-6 rounded-2xl bg-gradient-to-br from-muted/50 to-muted border border-border">
  ...
  return <Layout>
      <section className="py-12 lg:py-20">
        <div className="container mx-auto px-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            ...
            const rating = panditRatings?.[pandit.id];
            return <motion.div key={pandit.id} initial={{
              opacity: 0,
              y: 30
            }} animate={{
              opacity: 1,
              ...
              delay: 0.1 * Math.min(index, 5)
            }} className="group p-6 rounded-2xl bg-card border border-border hover:border-primary/30 hover:shadow-temple transition-all duration-300 cursor-pointer" onClick={() => setSelectedPandit(pandit)}>
                  <div className="flex items-start gap-4 mb-4">
                    <div className="relative">
                      {pandit.photo_url ? <img src={pandit.photo_url} alt={pandit.name} className="w-24 h-24 rounded-xl object-cover border-2 border-gold/30" /> : <div className="w-24 h-24 rounded-xl bg-muted flex items-center justify-center border-2 border-gold/30">
                    ...
                    <div className="flex-1">
                      ...
                      {experience && <div className="flex items-center gap-1 text-sm text-muted-foreground mb-1">
                          <Briefcase className="w-3.5 h-3.5" />
                          {experience} experience
                        </div>}
                      {/* Rating in directory */}
                      {rating && <div className="flex items-center gap-2 mt-1">
                  ...
                  {pandit.bio && <p className="text-sm text-muted-foreground mb-4 line-clamp-2">
                      {pandit.bio}
                    </p>}

                  <div className="flex flex-wrap gap-2 mb-4">
                    {pandit.expertise?.map((skill: string) => <Badge key={skill} variant="secondary" className="text-xs">
                        {skill}
                      </Badge>)}
                  </div>
                    ...
                    <TooltipProvider delayDuration={100}>
                      ...
                      start,
                            ...
                            <TooltipTrigger asChild>
                              <div className={`text-center p-1.5 rounded cursor-pointer transition-colors ${start ? 'bg-primary/10 text-primary hover:bg-primary/20' : 'bg-muted text-muted-foreground hover:bg-muted/80'}`}>
                  ...
                  {isVerified ? <div className="flex gap-2">
                      {pandit.phone && <Button variant="hero" size="sm" className="flex-1" onClick={() => window.open(`tel:${pandit.phone}`, '_self')}>
                          <Phone className="w-4 h-4 mr-1" />
                          Call
                        </Button>}
                      {pandit.whatsapp && <Button variant="golden" size="sm" className="flex-1" onClick={() => window.open(`https://wa.me/${pandit.whatsapp?.replace(/\D/g, '')}`, '_blank')}>
          ...
          {!isLoading && filteredPandits.length === 0 && <div className="text-center py-12">
      ...
      {/* Pandit Detail Modal */}
      <PanditDetailModal pandit={selectedPandit} open={!!selectedPandit} onOpenChange={open => !open && setSelectedPandit(null)} />
    </Layout>;
}